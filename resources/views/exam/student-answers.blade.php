    ...
    <div class="col-12">
        <div class="answers-header" style="padding: 1rem; margin-bottom: 1rem;">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h4 class="mb-0 fw-bold" style="font-size: 1.25rem;">Exam Results Overview</h4>
                    <p class="mb-0 opacity-75" style="font-size: 0.875rem;">Detailed breakdown of performance</p>
                </div>
                <div class="col-md-4 text-md-end">
                    <div class="score-card" style="padding: 1rem;">
                        <div class="score-badge" style="padding: 0.5rem 1rem; font-size: 0.875rem;">
                            <i class="ri-trophy-line me-1"></i>Score
                        </div>
                        <h3 class="mb-0 fw-bold" style="font-size: 1.5rem;">{{ $result->score }} / {{ $result->total_marks }}</h3>
                        <div class="text-muted small mt-1" style="font-size: 0.75rem;">
                            {{ round(($result->score / $result->total_marks) * 100, 1) }}% Achieved
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-header d-flex align-items-center bg-light">
                            <div class="flex-grow-1">
                                <h5 class="card-title mb-0">
                                    <i class="ri-file-text-line me-2 text-primary"></i>Question Answers
                                </h5>
                                            ...
                                            <tr class="border-bottom">
                                                ...
                                                <td>
                                                    <div class="question-html" data-full-text="{!! addslashes($qa->question_text) !!}">
                                                        {!! $qa->question_text !!}
                                                        @if(strlen(strip_tags($qa->question_text)) > 200) {{-- Rough check for long text --}}
                                                ...
                                                <td>
                                                    <div class="answer-cell bg-success bg-opacity-10 text-success fw-semibold">
                                                        {{ $qa->correct_answer ?? '-' }}
                                                    </div>
                                                </td>
                                                <td>
                                                    @php
                                                        $status = $qa->marked_correct;
                                                        $badgeClass = $status == 'Yes' ? 'correct-badge' : ($status == 'Not Attempted' ? 'not-attempted-badge' : 'incorrect-badge');
                                                        $icon = $status == 'Yes' ? 'ri-check-line' : ($status == 'Not Attempted' ? 'ri-time-line' : 'ri-close-line');
                                                    @endphp
