console.log("mysubjectvetting.init.js is loaded and executing!");
...
function filterData() {
    ...
    if (subjectVettingList) {
        subjectVettingList.filter(function(item) {
...
// Check if all scores are vetted for the selected vetting
...
function refreshTable() {
    ...
    .then(response => {
        ...
        if (subjectVettingList) {
            ...
            window.vettingData = response.data.subjectvettings.map(item => ({
                svid: item.svid,
...
if (editSubjectVettingForm) {
    editSubjectVettingForm.addEventListener("submit", function (e) {
        ...
        axios.put(`/mysubjectvettings/${id}`, { status }, {
        ...
        .then(function (response) {
            ...
            setTimeout(() => {
                Swal.fire({
                    position: "center",
                    icon: "success",
                    title: response.data.message || "Vetting status updated successfully!",
...
if (editModal) {
    editModal.addEventListener("show.bs.modal", function (event) {
        ...
        if (modalLabel) modalLabel.innerHTML = "Update Vetting Status";
        if (updateBtn) {
